...
import { executeGraphql, schemaSdl } from "../modules/graphql/execute.js";
...
export async function buildDeliveryTools(
  ...
  // GraphQL (18-IMPL F) — one query instead of N per-type calls: filters, ordering, relation
  // traversal and field selection in a single round trip. Not offered to locale-scoped tokens:
  // the query language carries its own `locale` argument, which the scope could not override.
  if (localeScope === null) {
    add(
      {
        name: "graphql_schema",
        description:
          "The GraphQL SDL for this workspace's published content (types, `<type>s(where, orderBy, limit, offset)` lists, `<type>(id)` lookups, single types). Read it once, then query with `graphql`.",
    ...
    add(
      {
        name: "graphql",
        description:
          "Run a read-only GraphQL query over published content. Supports where filters (eq/ne/in/contains/lt/gte/…), orderBy, limit ≤ 100, offset, relation traversal (depth ≤ 6) and field selection — prefer this over chaining *_list/*_get calls. Call graphql_schema first for the exact field names.",
        inputSchema: {
          type: "object",
          properties: {
            query: { type: "string", minLength: 1, description: "GraphQL query document (queries only)" },
            variables: { type: "object", description: "Variable values for the query" },
          },
          required: ["query"],
        },
      },
      (args) =>
        executeGraphql(dctx, {
          ...
          variables: args.variables && typeof args.variables === "object" ? (args.variables as Record<string, unknown>) : undefined,
        }),
    );
  }
