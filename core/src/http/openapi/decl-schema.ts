...
export const SCHEMA_OPS: OpDecl[] = [
  ...
  { method: "POST", path: "/api/taxonomies", tag: TAX, summary: "Create a taxonomy", input: taxonomyCreate.input, status: 201, returns: "The created taxonomy" },
  ...
  { method: "PATCH", path: "/api/taxonomy-nodes/:nodeId", tag: TAX, summary: "Edit a node: name, slug, attribute set", input: taxonomyNodeUpdate.input, returns: "The updated node", description: "A new slug rewrites the path of the node and of every node under it; 409 when a sibling already has that path." },
