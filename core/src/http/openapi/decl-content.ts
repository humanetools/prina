...
export const CONTENT_OPS: OpDecl[] = [
  {
    ...
    returns: "{ items[], pagination: { page, pageSize, total, pageCount } } — with `include=taxonomies` each item also carries taxonomies[]: { nodeId, name, path, taxonomy, attributes, entryComponentUid, entryComponentValues }",
  },
  {
  ...
  { method: "PUT", path: "/api/content/:typeUid/:id/taxonomies", tag: ENT, summary: "Replace the entry's taxonomy attachments", input: entrySetTaxonomies.input, returns: "{ entryId, attached }", description: "`attachments` is the full set (≤ 1,000) — `[]` detaches everything. `entryComponentValues` is validated against the node's entry component." },
  {
    method: "POST", path: "/api/content/:typeUid/:id/translate", tag: ENT, summary: "AI-translate an entry into one missing locale", input: entryAiTranslate.input, omit: ["sourceEntryId"], status: 201,
    description: "Creates a DRAFT sibling of the entry in `targetLocale` with its text translated by the workspace's AI provider (BYOK — the cost is the workspace's). Publishing stays a separate, human step. One locale per call; an existing locale is never overwritten (409). Taxonomy attachments are copied. `fields` narrows what is translated (logical paths such as `title`, `seo.meta_title`; default: the type's localizable text); `includeSeo=false` leaves the SEO record as is. The call lasts as long as the model takes — allow 120 s.",
    returns: "{ entry, translatedFields[], segmentCount, issues[] } — `entry.aiDraft` records the source entry, source locale and model; `issues` lists segments kept untranslated or trimmed to a field's maxLength",
    errors: { "400": "AiNotConfigured", "502": "AiProvider" },
  },
  {
  ...
  { method: "PATCH", path: "/api/assets/:id", tag: AST, summary: "Update asset metadata — alt text, folder (move)", input: assetUpdate.input, returns: "The updated asset" },
  {
    ...
    description: "`nodeIds` is the full set (≤ 1,000) — `[]` detaches everything. Assets share the taxonomies entries use; find assets by node with `GET /api/assets?taxonomy=`.",
