...
export function registerImportAiRoutes(
  ...
  // 31-IMPL — the models a provider offers, fetched with the caller's (or a stored) key
  app.post("/api/ai/models", async (req) => aiModelsList.run(req.body, await ctx(req)));
