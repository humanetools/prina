...
export function registerEntryRoutes(
  ...
  // AI translation into one missing locale (28-IMPL) — under the content path so access tokens reach it
  // with the entries grant (type-narrowed grants apply); the session-only /api/ai/translate stays for the admin
  app.post("/api/content/:typeUid/:id/translate", async (req, reply) => {
    const { typeUid, id } = req.params as { typeUid: string; id: string };
    return reply.status(201).send(
      await entryAiTranslate.run({ ...(req.body as object), typeUid, sourceEntryId: id }, await ctx(req)),
    );
  });
