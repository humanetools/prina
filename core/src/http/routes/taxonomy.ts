...
export function registerTaxonomyRoutes(
  ...
  // Edit name / description / attribute definitions (33-IMPL)
  app.patch("/api/taxonomies/:taxonomyUid", async (req) => {
    const { taxonomyUid } = req.params as { taxonomyUid: string };
    return taxonomyUpdate.run({ ...(req.body as object), uid: taxonomyUid }, await ctx(req));
  });
  // Delete a taxonomy with its tree — attachments cascade, entries and assets stay (29-IMPL)
  app.delete("/api/taxonomies/:taxonomyUid", async (req) => {
    const { taxonomyUid } = req.params as { taxonomyUid: string };
    return taxonomyDelete.run({ uid: taxonomyUid }, await ctx(req));
  });
  ...
  app.patch("/api/taxonomy-nodes/:nodeId", async (req) => {
    const { nodeId } = req.params as { nodeId: string };
    return taxonomyNodeUpdate.run({ ...(req.body as object), nodeId }, await ctx(req));
  });
