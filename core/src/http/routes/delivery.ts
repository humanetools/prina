 ...
 *    (&embed=1 → JSON for embed.js {html,css,js,ga}; &populate=1 → relations/media inline in the Liquid scope)
...
import {
  buildTaxonomyConditions,
  getDeliveryTaxonomy,
  listDeliveryTaxonomies,
  loadEntryTaxonomies,
  parseTaxonomyParams,
} from "../../modules/delivery/taxonomy.js";
...
import { executeGraphql, schemaSdl, wireSchemaInvalidation } from "../../modules/graphql/execute.js";
...
export function registerDeliveryRoutes(
  ...
  // 18-IMPL A — GraphQL read over the same plane: schema generated from the workspace's types,
  // filters/populate/draft-token semantics identical to the REST routes below. Read-only.
  wireSchemaInvalidation(services.events);
  const gqlBody = (body: unknown, query: Record<string, unknown>) => {
    const b = (body && typeof body === "object" ? body : {}) as { query?: unknown; variables?: unknown; operationName?: unknown };
    const q = typeof b.query === "string" ? b.query : typeof query.query === "string" ? query.query : "";
    let variables: Record<string, unknown> | null = null;
    if (b.variables && typeof b.variables === "object") variables = b.variables as Record<string, unknown>;
    else if (typeof query.variables === "string") { try { variables = JSON.parse(query.variables) as Record<string, unknown>; } catch { variables = null; } }
    return { query: q, variables, operationName: typeof b.operationName === "string" ? b.operationName : null };
  };
  app.options("/delivery/graphql", async (_req, reply) =>
    reply.status(204).header("access-control-allow-methods", "POST, GET, OPTIONS").header("access-control-allow-headers", "content-type").send(),
  );
  const handleGraphql = async (req: { body?: unknown; query: unknown }, reply: import("fastify").FastifyReply) => {
    const query = req.query as Record<string, unknown>;
    const ctx = await buildCtx(query);
    const g = gqlBody(req.body, query);
    if (!g.query.trim()) return reply.status(400).send({ errors: [{ message: "Missing GraphQL query" }] });
    const result = await executeGraphql(ctx, g);
    return reply.header("cache-control", "no-store").send(result);
  };
  app.post("/delivery/graphql", async (req, reply) => handleGraphql(req, reply));
  app.get("/delivery/graphql", async (req, reply) => handleGraphql(req, reply));
  /** SDL of the generated schema — what tools and models read */
  app.get("/delivery/graphql/schema", async (req, reply) => {
    const ctx = await buildCtx(req.query as Record<string, unknown>);
    return reply.header("content-type", "text/plain; charset=utf-8").header("cache-control", "no-store").send(await schemaSdl(ctx));
  });
  ...
  // Taxonomy (22-IMPL) — static routes, so they win over /delivery/:typeUid
  app.get("/delivery/taxonomies", async (req, reply) => {
    const ctx = await buildCtx(req.query as Record<string, unknown>);
    return reply.header("cache-control", cacheHeader(ctx)).send(await listDeliveryTaxonomies(ctx));
  });
  app.get("/delivery/taxonomies/:uid", async (req, reply) => {
    const { uid } = req.params as { uid: string };
    const ctx = await buildCtx(req.query as Record<string, unknown>);
    return reply.header("cache-control", cacheHeader(ctx)).send(await getDeliveryTaxonomy(ctx, uid));
  });
  ...
  app.get("/delivery/:typeUid", async (req, reply) => {
    ...
    const query = req.query as Record<string, unknown>;
    const ctx = await buildCtx(query);
    // ?taxonomy=<uid>:<path> (repeatable, ANDed) — descendants included unless taxonomyExact=1
    const taxonomyConds = await buildTaxonomyConditions(ctx, parseTaxonomyParams(query.taxonomy), query.taxonomyExact === "1");
    ...
    if (query.taxonomies === "1" && items.length > 0) {
      const byEntry = await loadEntryTaxonomies(ctx, items.map((i) => i.id), { populate: query.populate === "1" });
      for (const item of items as Array<Record<string, unknown>>) item.taxonomies = byEntry.get(item.id as string) ?? [];
    }
    reply
  ...
  app.get("/delivery/:typeUid/:id", async (req, reply) => {
    ...
    if (query.format !== "html") {
      ...
      const entryTaxonomies =
        query.taxonomies === "1"
          ? (await loadEntryTaxonomies(ctx, [resolved.entry.id], { populate: query.populate === "1" })).get(resolved.entry.id) ?? []
          : undefined;
      return reply.header("cache-control", cacheHeader(ctx)).send({
        ...
        values,
        ...(entryTaxonomies ? { taxonomies: entryTaxonomies } : {}),
