    ...
    const paths: Record<string, unknown> = {
      ...
      "/delivery/graphql": {
        post: {
          ...
          description:
            "Read-only. Per type: `<types>(where, orderBy, limit ≤ 100, offset, locale) { total items { … } }`, `<type>(id)`, and single types as " +
            ...
            "Selections populate relations/media; nesting depth ≤ 6. Drafts appear only with a valid `draft` token. Fetch the SDL from /delivery/graphql/schema.",
          tags: ["delivery"],
