...
export function registerOpenApiRoutes(
  ...
  app.get("/openapi.json", async (req) => {
    ...
    const paths: Record<string, unknown> = {
      ...
      "/delivery/graphql": {
        post: {
          ...
          description:
            ...
            "Selections populate relations/media; nesting depth ≤ 6. Every entry has `taxonomies { taxonomy path name attributes … }`; roots `taxonomies` and `taxonomy(uid)` serve the trees. Drafts appear only with a valid `draft` token. Fetch the SDL from /delivery/graphql/schema.",
          tags: ["delivery"],
      ...
      "/delivery/taxonomies": {
        get: { summary: "Taxonomies of this workspace", tags: ["delivery"], parameters: [wsParam], responses: { "200": { description: "{ items: [{ uid, name, description }] }" } } },
      },
      "/delivery/taxonomies/{uid}": {
        get: {
          ...
          responses: {
            "200": { description: "{ uid, name, description, nodes: [{ id, parentId, name, slug, path, depth, position, attributeSet }] }" },
      ...
      paths[`/delivery/${type.uid}`] = {
        get: {
          ...
          parameters: [
            ...
            q("taxonomies", "1 = add `taxonomies[]` to each item: { taxonomy, nodeId, name, slug, path, attributeSet, attributes } (attributes resolved when populate=1)"),
            q(
              "filters[field][$op]",
      ...
      paths[`/delivery/${type.uid}/{id}`] = {
        get: {
          ...
          parameters: [
            ...
            q("taxonomies", "1 = add `taxonomies[]` — the nodes this entry sits in, with attribute-set values (json format)"),
          ],
