...
const envSchema = z.object({
  ...
  /**
   * Which plane this instance serves (split deployment: admin on the intranet, delivery on the
   * public net, same image + same DB/S3). `all` = today's single instance. `delivery` answers
   * only the public surface (/delivery, /img, /mcp/delivery, /.well-known, /health, /) — every
   * admin/management route is 404 at the core, not just at the proxy — and runs no embedding
   * worker. `admin` serves everything and owns the workers.
   */
  PRINA_PLANE: z.enum(["all", "admin", "delivery"]).default("all"),
