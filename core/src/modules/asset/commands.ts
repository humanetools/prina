...
import { isExact, parseTaxonomyParams } from "../taxonomy/filter.js";
import { assetTaxonomyConditions, loadAssetTaxonomies } from "./taxonomy-commands.js";
import { FOLDER_PATH_RE } from "./folder-path.js";
import { collectFolderPaths, rememberFolders } from "./folder-commands.js";
...
export const assetRequestUpload = defineCommand({
  ...
  async execute(input, ctx) {
    ...
    const [asset] = await ctx.db
      ...
      .returning();
    await rememberFolders(ctx, [input.folder]);
...
/** List — folder/search/taxonomy filters + usage counts (T4.4, 23-IMPL) */
export const assetList = defineCommand({
  ...
  input: z.object({
    ...
    taxonomy: z
      ...
      .optional()
      .describe("`<taxonomyUid>:<node.path>` — assets under that node, descendants included. Repeatable, ANDed (also with `folder`)."),
    taxonomyExact: z
      .union([z.boolean(), z.enum(["0", "1", "false", "true"])])
      .optional()
      .describe("`1` = only the node itself, not its descendants"),
  ...
  async execute(input, ctx) {
    ...
    conds.push(...(await assetTaxonomyConditions(ctx, parseTaxonomyParams(input.taxonomy), isExact(input.taxonomyExact))));
...
/**
 * Folder list — plain path strings, kept for existing consumers. Same set as `asset.folder_tree`
 * (registered ∪ holding assets ∪ ancestors), plus `/` while assets are filed there.
 */
export const assetFolders = defineCommand({
  ...
  async execute(_input, ctx) {
    ...
    const [root] = await ctx.db
      .select({ id: assets.id })
      .from(assets)
      .where(and(eq(assets.workspaceId, ctx.workspaceId), eq(assets.folder, "/")))
      .limit(1);
    if (root) paths.add("/");
    return [...paths].sort();
  },
...
export const assetGet = defineCommand({
  ...
  async execute(input, ctx) {
    ...
    return {
      ...asset,
      usages,
      taxonomies: (await loadAssetTaxonomies(ctx, [asset.id])).get(asset.id) ?? [],
...
/**
 * Update editable asset metadata — alt text (a11y, WCAG 1.1.1) and the folder the asset is filed in.
...
export const assetUpdate = defineCommand({
  ...
  input: z.object({
    ...
    /** Move the asset — the folder is created when it does not exist yet */
    folder: z.string().max(400).regex(FOLDER_PATH_RE, "Folder path format: /a/b").optional(),
  }),
  permission: mediaPermission(PermissionAction.Update),
  async execute(input, ctx) {
    if (input.alt === undefined && input.folder === undefined) {
      throw new ValidationError("Nothing to update — send `alt` and/or `folder`");
    }
    const asset = await getAssetScoped(ctx, input.id);
    const moving = input.folder !== undefined && input.folder !== asset.folder;
    // the folder being left is registered too, so it stays in the tree when this was its last asset
    if (moving) await rememberFolders(ctx, [asset.folder, input.folder!]);
    const [updated] = await ctx.db
      .update(assets)
      .set({
        ...(input.alt !== undefined ? { alt: input.alt } : {}),
        ...(moving ? { folder: input.folder } : {}),
        updatedAt: new Date(),
      })
      .where(eq(assets.id, asset.id))
      .returning();
  ...
  resourceId: (i) => i.id,
  auditPayload: (i) => ({
    ...(i.alt !== undefined ? { described: i.alt !== null && i.alt.length > 0 } : {}),
...
export const assetDelete = defineCommand({
  ...
  async execute(input, ctx) {
    ...
    await rememberFolders(ctx, [asset.folder]);
