...
export interface GqlContext {
  ...
  list(typeUid: string, args: { locale?: string; limit: number; offset: number; specs: FilterSpec[]; sort: SortSpec[]; taxonomy?: string[]; taxonomyExact?: boolean }): Promise<{ items: Record<string, unknown>[]; total: number }>;
  ...
  /** Taxonomy (22-IMPL) — per-entry nodes are batched into one query per tick */
  entryTaxonomies(entryId: string): Promise<EntryTaxonomy[]>;
  taxonomies(): Promise<Array<{ uid: string; name: string; description: string | null }>>;
  taxonomy(uid: string): Promise<Awaited<ReturnType<typeof getDeliveryTaxonomy>> | null>;
}
...
export function createGqlContext(delivery: DeliveryCtx): GqlContext {
  ...
  // entry → taxonomies, batched: resolvers of one list page ask in the same tick
  let waiting: Array<{ id: string; resolve(v: EntryTaxonomy[]): void; reject(e: unknown): void }> = [];
  const flush = () => {
    const batch = waiting;
    waiting = [];
    loadEntryTaxonomies(delivery, [...new Set(batch.map((b) => b.id))], { populate: true }).then(
      (byEntry) => batch.forEach((b) => b.resolve(byEntry.get(b.id) ?? [])),
  ...
  return {
    delivery,
    entryTaxonomies(entryId) {
      return new Promise((resolve, reject) => {
        if (waiting.push({ id: entryId, resolve, reject }) === 1) process.nextTick(flush);
      });
    },
    async taxonomies() {
      return (await listDeliveryTaxonomies(delivery)).items;
    },
    async taxonomy(uid) {
      try {
        return await getDeliveryTaxonomy(delivery, uid);
      } catch {
        return null;
      }
    },
    async list(typeUid, args) {
      ...
      const taxonomyConds = await buildTaxonomyConditions(delivery, parseTaxonomyParams(args.taxonomy), args.taxonomyExact === true);
      ...
      const res = await deliveryList(delivery, typeUid, args.locale, { page: aligned ? page : 1, pageSize: aligned ? pageSize : Math.min(100, args.offset + pageSize) }, args.specs, args.sort, taxonomyConds);
