...
import { TAXONOMY_TYPE_NAMES, entryTaxonomiesField, taxonomyQueryFields } from "./taxonomy-types.js";
...
export function buildSchema(types: GqlTypeRow[], components: Map<string, ContentTypeDefinition>): GraphQLSchema {
  ...
  const usedNames = new Set<string>(["Media", "RichText", "JSON", "Query", "OrderDir", "StringFilter", "NumberFilter", "BooleanFilter", "IdFilter", ...TAXONOMY_TYPE_NAMES]);
    ...
    objectTypes.set(
      ...
      new GraphQLObjectType({
        ...
        fields: () => ({
          ...
          // after the value fields so a content field named `taxonomies` is replaced, not duplicated
          taxonomies: entryTaxonomiesField,
        }),
      }),
    );
  }
  ...
  for (const t of types) {
    ...
    queryFields[claim(plural(c), `${c}List`)] = {
      ...
      args: {
        ...
        taxonomy: { type: new GraphQLList(new GraphQLNonNull(GraphQLString)), description: "`<taxonomyUid>:<node.path>` — entries under that node (ANDed when several)" },
        taxonomyExact: { type: GraphQLBoolean, defaultValue: false, description: "Only the node itself, not its descendants" },
      },
      resolve: (_s, args: { where?: Record<string, Record<string, unknown>>; orderBy?: Array<{ field: string; dir?: "asc" | "desc" }>; limit?: number; offset?: number; locale?: string; taxonomy?: string[]; taxonomyExact?: boolean }, ctx) =>
        ctx.list(t.uid, {
          locale: args.locale,
          taxonomy: args.taxonomy,
          taxonomyExact: args.taxonomyExact,
  ...
  Object.assign(queryFields, taxonomyQueryFields({ list: claim("taxonomies", "taxonomyTrees"), one: claim("taxonomy", "taxonomyTree") }));
