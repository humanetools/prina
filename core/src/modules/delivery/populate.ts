 ...
 *   (`withValues` adds the target's own `values`, depth 1 — the target's relations stay ids)
...
import { findRelationPath } from "../../content/relation-paths.js";
...
export interface RelSummary {
  ...
  /** Target entry values (raw, depth 1) — only with `withValues` (template render scope) */
  values?: Record<string, unknown>;
}

export interface PopulateOptions {
  /** Inline the target entry's values on relation summaries (Liquid templates render lists from it) */
  withValues?: boolean;
}
...
export async function collectInverseEdges(
  ...
  // `field` is a relation path — may point inside a component / dynamic zone (hand-off-02)
  const componentMap = rows.length > 0 ? await loadComponentMap(ctx.db, ctx.workspace.id) : new Map();
  for (const row of rows) {
    const def = findRelationPath(row.typeDefinition, componentMap, row.field)?.def as
      | { writeInverse?: boolean; inversePredicate?: string }
      | undefined;
  ...
  for (const [targetUid, ids] of relIdsByTarget) {
    ...
    for (const row of rows) {
      ...
      relMap.set(row.id, {
        ...
        type: row.typeUid,
        schemaOrgType: row.schemaOrgType,
        ...(options.withValues ? { values: row.values as Record<string, unknown> } : {}),
      });
    }
  }
