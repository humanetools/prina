 ...
 *
 * Three reads: the classification tree (nodes with their attributes), a filter for entry lists
 * ("everything under this node"), and the nodes an entry sits in together with its entry-component
 * values. Naming (33-IMPL): `attributes` = the node's own values, `entryComponent(Values)` = what the
 * entry filled in for that node. Nodes have no draft state —
...
export interface EntryTaxonomy {
  ...
  slug: string;
  path: string;
  /** the node's own attribute values (taxonomy attributeFields) */
  attributes: Record<string, unknown>;
  /** component uid of the node's entry component */
  entryComponent: string | null;
  /** what this entry filled in for the node's entry component */
...
export async function listDeliveryTaxonomies(ctx: DeliveryCtx) {
  const rows = await ctx.db
    .select({ uid: taxonomies.uid, name: taxonomies.name, description: taxonomies.description, attributeFields: taxonomies.attributeFields })
    .from(taxonomies)
...
export async function getDeliveryTaxonomy(ctx: DeliveryCtx, uid: string) {
  ...
  return {
    ...
    attributeFields: taxonomy.attributeFields,
    nodes: nodes.map((n) => ({
      id: n.id,
      parentId: n.parentId,
      name: n.name,
      ...
      entryComponent: n.entryComponentUid,
    })),
  };
}

/** Nodes (with attributes and entry-component values) of each entry — one query for the whole page of entries */
export async function loadEntryTaxonomies(
  ...
  const rows = await ctx.db
    .select({
      ...
      entryComponent: taxonomyNodes.entryComponentUid,
  ...
  // populate: resolve media / relations inside entry-component values with the component's own definition
  const resolved = new Map<number, Record<string, unknown>>();
  if (opts.populate) {
    const uids = [...new Set(rows.filter((r) => r.entryComponentValues && r.entryComponent).map((r) => r.entryComponent!))];
    ...
    for (const def of defs) {
      const idx = rows.map((r, i) => (r.entryComponent === def.uid && r.entryComponentValues ? i : -1)).filter((i) => i >= 0);
      const values = await populateValuesList(ctx, def.definition as ContentTypeDefinition, idx.map((i) => rows[i]!.entryComponentValues!));
  ...
  rows.forEach((r, i) => {
    out.get(r.entryId)?.push({
      ...
      entryComponent: r.entryComponent,
      entryComponentValues: resolved.get(i) ?? r.entryComponentValues ?? null,
    });
  });
  return out;
}
