...
export const templateRenderPreview = defineCommand({
  ...
  input: z.object({
    ...
    /** mirror of delivery `?populate=1` — relations/media inline (targets carry `values`) */
  ...
  async execute(input, ctx) {
    ...
    // Preview includes drafts, so populated targets may be drafts too (delivery serves published only)
    const scopeValues = input.populate
      ? (await populateValuesList(
          { db: ctx.db, services: ctx.services, workspace: { id: ctx.workspaceId, slug: "", settings: {} }, includeDraft: true },
