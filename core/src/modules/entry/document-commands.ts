...
/** Multi-taxonomy attach — full-replace semantics. Entry component values validated against the component definition (§2.8) */
export const entrySetTaxonomies = defineCommand({
    ...
    attachments: z
      .array(
        z.object({
          ...
          entryComponentValues: z.record(z.unknown()).nullable().default(null),
        }),
      )
      .max(1000), // same ceiling as assets — an entry can be classified under hundreds of nodes
  }),
  permission: (i) => ({
  ...
  async execute(input, ctx) {
    ...
    // Entry component value validation (T2.5): uses the node's entryComponentUid component definition
    for (const attach of input.attachments) {
      const node = nodeMap.get(attach.nodeId)!;
      if (!attach.entryComponentValues) continue;
      if (!node.entryComponentUid) {
        throw new ValidationError(
          `Node '${node.name}' has no entry component — entryComponentValues cannot be sent`,
        );
      }
      const [comp] = await ctx.db
        ...
        .where(
          and(
            ...
            eq(components.uid, node.entryComponentUid),
          ),
        )
        .limit(1);
      if (!comp) {
        throw new ValidationError(
          `Entry component '${node.entryComponentUid}' not found`,
        );
      }
      ...
      assertValuesAgainstSchema(compiled, attach.entryComponentValues as Record<string, unknown>);
    }
    ...
    if (input.attachments.length > 0) {
      await ctx.db.insert(entryTaxonomyNodes).values(
        input.attachments.map((a) => ({
          ...
          entryId: entry.id,
          nodeId: a.nodeId,
          entryComponentValues: a.entryComponentValues,
        })),
      );
    }
