...
import { isExact, parseTaxonomyParams, taxonomyLinkConditions } from "../taxonomy/filter.js";
import { loadEntryAttachments, type EntryTaxonomyAttachment } from "./taxonomy-read.js";
...
export const entryGet = defineCommand({
  ...
  async execute(input, ctx) {
    ...
    // Taxonomy attach + entry components (T2.5) — same loader the list uses, so both return one shape
    const attached = (await loadEntryAttachments(ctx, [entry.id])).get(entry.id) ?? [];
...
/** What `entry.list` can add to each item on request (`include=`) */
const LIST_INCLUDES = ["taxonomies"];
...
export const entryList = defineCommand({
  ...
  input: z.object({
    ...
    taxonomy: z
      ...
      .optional()
      .describe("`<taxonomyUid>:<node.path>` — entries attached under that node, descendants included. Repeatable, ANDed with each other and with every other filter; `pagination.total` follows."),
    taxonomyExact: z
    ...
    include: z
      .union([z.string(), z.array(z.string())])
      .optional()
      .describe("Extra data per item, comma-separated or repeated. `taxonomies` adds `taxonomies[]` in the shape of the single-entry read — one query for the page, so no request per row."),
  ...
  async execute(input, ctx) {
    ...
    conds.push(
      ...(await taxonomyLinkConditions(ctx, parseTaxonomyParams(input.taxonomy), isExact(input.taxonomyExact), {
        table: "entry_taxonomy_nodes", fk: "entry_id", id: entries.id,
      })),
    );
    // A typo here must not pass silently — the caller would fall back to one request per row
    const includes = new Set([input.include ?? []].flat().flatMap((v) => v.split(",")).map((v) => v.trim()).filter(Boolean));
    const unknownInclude = [...includes].filter((v) => !LIST_INCLUDES.includes(v));
    if (unknownInclude.length > 0) {
      throw new ValidationError(`Unknown include '${unknownInclude.join(", ")}' — supported: ${LIST_INCLUDES.join(", ")}`);
    }
    ...
    const attachments = includes.has("taxonomies") ? await loadEntryAttachments(ctx, masked.map((r) => r.id)) : null;
    return {
      // `taxonomies` is present only when asked for — the plain list keeps its shape
      items: masked.map((r): typeof r & { taxonomies?: EntryTaxonomyAttachment[] } =>
        attachments ? { ...r, taxonomies: attachments.get(r.id) ?? [] } : r),
      pagination: {
        page: input.page,
