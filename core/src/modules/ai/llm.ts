...
/** An image attached to the user turn (15-IMPL G2 multimodal) — base64 body + mime */
...
export interface LlmRequest {
  ...
  user: string;
  maxTokens?: number;
  /** Vision input — only providers flagged in PROVIDER_VISION accept it (callers ignore otherwise) */
  images?: LlmImage[];
}
...
/** Providers whose chat API accepts image parts (15-IMPL G2) — the router filters on this */
export const PROVIDER_VISION: Record<AiProvider, boolean> = {
  anthropic: true,
  openai: true,
  gemini: true,
  mistral: false,
...
function anthropicCaller(settings: AiSettings): LlmCaller {
  return async ({ system, user, maxTokens = 4096, images }) => {
