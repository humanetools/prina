...
- Liquid templates (Settings › Templates) expose the scope values / entry / type and exactly these custom filters: asset_url, asset_alt, richtext_to_html, won, date_ko. A richtext value is ProseMirror JSON — it must go through richtext_to_html. Never invent filter names.
...
export async function runAssistantLoop(
  ...
  emit?: (e: AssistantEvent) => void,
    ...
    for (let round = 0; round < MAX_ROUNDS; round++) {
      ...
      messages.push({ kind: "tool_calls", text: turn.text, calls: turn.toolCalls, ...(turn.raw !== undefined ? { raw: turn.raw } : {}) });
