...
/**
 * Models a provider offers right now (31-IMPL) — for the admin picker. Either a key typed into the
 * connect modal, or `entryId` of a connected entry (its stored key is used and never returned).
 */
export const aiModelsList = defineCommand({
  name: "ai_models.list",
  resource: "ai_settings",
  skipAudit: true,
  input: z.object({
    ...
    baseUrl: z.string().url().optional(),
    anthropicWorkspaceId: z.string().trim().max(200).optional(),
    /** a connected entry — provider / key / baseUrl come from what is stored */
    entryId: z.string().min(1).max(40).optional(),
  }),
  permission: () => ({ action: PermissionAction.Update, subject: SystemSubject.Settings }),
  async execute(input, ctx) {
    let req = { target: input.target, provider: input.provider ?? "", apiKey: input.apiKey, baseUrl: input.baseUrl, anthropicWorkspaceId: input.anthropicWorkspaceId };
    if (input.entryId) {
      const cfg = await readAiConfig(ctx.db);
      const chain: Array<LmChainEntry | SsChainEntry> = input.target === "lm" ? cfg.lm.chain : cfg.ss.chain;
      const e = chain.find((x) => x.id === input.entryId);
      if (!e) throw new NotFoundError(`No ${input.target} entry '${input.entryId}'`);
      req = {
        target: input.target, provider: e.provider, apiKey: input.apiKey || e.apiKey, baseUrl: input.baseUrl ?? e.baseUrl,
        anthropicWorkspaceId: input.anthropicWorkspaceId ?? (e as LmChainEntry).anthropicWorkspaceId,
      };
    }
    if (!req.provider) throw new ValidationError("Send `provider` (with a key) or `entryId`");
