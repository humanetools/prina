...
export type AgentMessage =
  ...
  /**
   * `raw` = the provider's own assistant message, exactly as it answered (Anthropic content blocks,
   * OpenAI message, Gemini content). Replayed verbatim on the next request: providers attach fields
   * to these parts that must come back unchanged (Gemini 3 thoughtSignature, thinking blocks,
   * reasoning items) — normalising to ToolCall and rebuilding would drop them, and each new model
   * would need its own field. Absent (fake LLMs, tests) → rebuilt from text/calls.
   */
  | { kind: "tool_calls"; text: string | null; calls: ToolCall[]; raw?: unknown }
  | { kind: "tool_results"; results: ToolResult[] };

export interface AgentTurn {
  text: string | null;
  toolCalls: ToolCall[];
  /** provider's assistant message as received — see AgentMessage.raw */
  raw?: unknown;
}
...
function anthropicToolCaller(settings: AiSettings): LlmToolCaller {
  return async ({ system, messages, tools, maxTokens = 4096 }) => {
    ...
    return { text: text || null, toolCalls, raw: data.content };
  };
}
...
function openaiToolCaller(settings: AiSettings): LlmToolCaller {
  return async ({ system, messages, tools, maxTokens = 4096 }) => {
    ...
    return { text: msg?.content || null, toolCalls, raw: msg };
  };
}
...
function geminiToolCaller(settings: AiSettings): LlmToolCaller {
  return async ({ system, messages, tools, maxTokens = 4096 }) => {
    ...
    // the content object (role + parts, thoughtSignature and all) is what goes back next turn
    return { text: text || null, toolCalls, ...(content ? { raw: { role: "model", parts } } : {}) };
  };
}
