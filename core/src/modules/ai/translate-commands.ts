  ...
  async execute(input, ctx) {
    ...
    if (Object.keys(segments).length > 0) {
      ...
      try {
        ...
        // A provider failure is not this server's fault and not the caller's input — API callers
        // need to tell it apart from a 500 (28-IMPL)
        throw new AppError(
          "AI_PROVIDER_ERROR",
          `The AI provider failed: ${(err instanceof Error ? err.message : String(err)).slice(0, 300)}`,
          502,
        );
      }
