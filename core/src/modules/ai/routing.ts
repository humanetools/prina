...
import {
  ...
  type AiSettings,
  type LlmCaller,
 PROVIDER_VISION } from "./llm.js";
import { IMAGE_MODELS, PROVIDER_IMAGE, createImageCaller, type ImageGenerator } from "./image.js";
import {
  EMBEDDING_DEFAULTS,
  embedTexts,
...
export async function createLlmRouter(
  db: Db,
  opts: { /** 15-IMPL G2 — only providers whose API accepts image parts */ vision?: boolean } = {},
): Promise<LlmCaller | null> {
  const cfg = await readAiConfig(db);
  const chain = opts.vision ? cfg.lm.chain.filter((e) => PROVIDER_VISION[e.provider]) : cfg.lm.chain;
  if (chain.length === 0) return null;
  return async (req) => {
    const { failover, retries, timeoutSec } = cfg.lm;
    const candidates = pickCandidates({ ...cfg.lm, chain });
...
/**
 * Image router (15-IMPL G3) — walks the LM chain restricted to providers with an image API,
 * swapping each entry's chat model for the provider's image model. Same health/failover
 * bookkeeping as the LM router. Null = no image-capable provider is connected.
 */
export async function createImageRouter(db: Db): Promise<ImageGenerator | null> {
  const cfg = await readAiConfig(db);
  const chain = cfg.lm.chain.filter((e) => PROVIDER_IMAGE[e.provider] && e.apiKey);
  if (chain.length === 0) return null;
  return async (req) => {
    const { failover, retries, timeoutSec } = cfg.lm;
    const candidates = pickCandidates({ ...cfg.lm, chain });
    let lastErr: unknown = new Error("No image-capable AI provider is connected");
    for (const e of candidates) {
      const caller = createImageCaller({ ...lmEntrySettings(e), model: IMAGE_MODELS[e.provider] ?? e.model });
      if (!caller) continue;
      try {
        // Image generation is slow — give it a wider window than chat
        const out = await withTimeout(caller(req), Math.max(timeoutSec, 90), `${entryLabel(e)} (image)`);
        if (noteSuccess(e.id).restored) {
          void appendRoutingLog(db, "ok", `${entryLabel(e)} healthy again on recheck — restored`);
        }
        return out;
        ...
        lastErr = err;
        if (noteFailure(e.id, retries).wentDown) {
          void appendRoutingLog(db, "warn", `${entryLabel(e)} stopped responding (image)${failover ? " — trying the next provider" : ""}`);
        }
        if (!failover) break;
      }
    }
