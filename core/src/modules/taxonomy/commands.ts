...
import { components, taxonomies, taxonomyNodes } from "../../db/schema/index.js";
...
/**
 * Edit a node in place — display name, attribute set, slug. A new slug rewrites the path of the
 * node and of everything under it (same ltree rewrite as a move); re-parenting is `taxonomy_node.move`.
 */
export const taxonomyNodeUpdate = defineCommand({
  ...
  input: z.object({
    ...
    attributeComponentUid: z.string().nullable().optional(),
  }),
  ...
  async execute(input, ctx) {
    ...
    if (input.attributeComponentUid) {
      const [comp] = await ctx.db
        .select({ id: components.id })
        .from(components)
        .where(and(eq(components.workspaceId, ctx.workspaceId), eq(components.uid, input.attributeComponentUid)))
        .limit(1);
      if (!comp) throw new ValidationError(`Component '${input.attributeComponentUid}' does not exist`);
    }
    ...
    if (input.attributeComponentUid !== undefined) patch.attributeComponentUid = input.attributeComponentUid;
  ...
  auditPayload: (i) => ({ name: i.name, slug: i.slug, attributeComponentUid: i.attributeComponentUid }),
});
