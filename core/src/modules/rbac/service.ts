...
export async function loadActorPermissions(
  ...
  // REST access tokens carry their own rights (grants → permission rows) — no role involved
  if (ctx.actor.type === ActorType.Ai && ctx.actor.permissions) return ctx.actor.permissions;
