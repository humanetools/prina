...
> &
  ...
  Partial<Pick<Env, "TRUST_PROXY" | "IMGPROXY_PUBLIC_URL" | "ONE_TIME_LOGIN_PUBLIC_KEY" | "PRINA_PLANE">>;

/**
 * Public surface of the delivery plane (PRINA_PLANE=delivery). Prefix match on the path
 * (query stripped); everything else is 404 before any route or auth hook runs.
 */
const DELIVERY_PLANE_PREFIXES = [
  "/health",
  "/delivery/",
  "/img/",
  "/mcp/delivery",
  "/.well-known/",
  // Flow SPA (EE) — root takeover + its assets; the landing page when Flow Web is off
  "/flow",
  "/flow-assets/",
  // Chatbot widget (EE) — public embed surface
  "/chat",
  "/chat-frame",
  "/chatbot.js",
] as const;
export function isDeliveryPlanePath(url: string): boolean {
  const path = url.split("?")[0] ?? "";
  if (path === "/") return true;
  return DELIVERY_PLANE_PREFIXES.some((p) => path === p || path.startsWith(p) || path === p.replace(/\/$/, ""));
}
...
declare module "fastify" {
  interface FastifyInstance {
    /** Registered routes (method + url) — the 20-IMPL route-coverage test asserts each /api route has a scope decision */
...
export function buildApp(opts: BuildAppOptions): FastifyInstance {
  ...
  // Split deployment (PRINA_PLANE): a delivery instance answers only the public surface.
  // Registered before the auth hooks so admin paths are 404 without touching the session table.
  const plane = opts.env.PRINA_PLANE ?? "all";
  if (plane === "delivery") {
    app.addHook("onRequest", async (req, reply) => {
      if (isDeliveryPlanePath(req.url)) return;
      return reply.status(404).send({ error: { code: "NOT_FOUND", message: "Route not found", details: null } });
    });
  }

  // Route table — every registered method+url (tests assert each /api route has a scope decision, 20-IMPL)
  const routeTable: Array<{ method: string; url: string }> = [];
  app.addHook("onRoute", (r) => {
    for (const m of Array.isArray(r.method) ? r.method : [r.method]) routeTable.push({ method: m, url: r.url });
  });
  ...
  registerMcpRoutes(app, opts.db, services, { connectPublicKey: opts.env.ONE_TIME_LOGIN_PUBLIC_KEY });
  ...
  if (plane !== "delivery" && existsSync(adminDist)) {
  ...
  app.log.info({ plane }, plane === "all" ? "Serving all planes" : `Serving the ${plane} plane`);
  return app;
}
