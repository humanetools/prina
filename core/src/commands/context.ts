...
export interface Actor {
  ...
  roleIds?: string[];
  /**
   * Explicit permission rows for ai actors (REST access tokens, 20-IMPL) — when present they
   * replace role lookup: the token's grants are its rights.
   */
  permissions?: Array<{ action: string; subject: string; fields: string[] | null; locales: string[] | null }>;
}
