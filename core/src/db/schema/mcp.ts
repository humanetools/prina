...
export const mcpTokens = pgTable(
  "mcp_tokens",
  {
    ...
    /** management plane: REST surface per resource group, e.g. {"content":"edit","schema":"read"} — NULL = legacy {content: edit} (20-IMPL) */
    scopes: jsonb("scopes").$type<Record<string, string>>(),
    /** management plane, custom mode: explicit grants [{item, level, types?}] — when non-empty the token needs no role (20-IMPL) */
    grants: jsonb("grants").$type<Array<{ item: string; level: string; types?: string[] }>>(),
