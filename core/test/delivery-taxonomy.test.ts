...
beforeAll(async () => {
  ...
  const attach = (id: string, attachments: Array<{ nodeId: string; attributeValues?: Record<string, unknown> }>) =>
    entrySetTaxonomies.run({ typeUid: "tx_item", id, attachments }, t.ctx);
  await attach(ids.charger, [{ nodeId: nodes.chargers, attributeValues: { wattage: 45, made_by: ids.brand } }, { nodeId: nodes.launch }]);
  await attach(ids.caseItem, [{ nodeId: nodes.cases }]);
  await attach(ids.phone, [{ nodeId: nodes.phones }, { nodeId: nodes.launch }]);
  await attach(ids.draftCharger, [{ nodeId: nodes.chargers, attributeValues: { wattage: 100 } }]);
});
afterAll(async () => {
  await app.close();
  await t.cleanup();
...
describe("delivery taxonomy — tree", () => {
  it("lists taxonomies and returns a path-ordered flat tree with parents first", async () => {
    ...
    expect(chargers).toMatchObject({ parentId: nodes.accessories, depth: 1, attributeSet: "tx_charger_attrs" });
...
describe("delivery taxonomy — on the entry", () => {
  it("is opt-in; attribute values come back stored, and resolved with populate=1", async () => {
    ...
    expect(chargers).toMatchObject({ name: "Chargers", slug: "fast-chargers", attributeSet: "tx_charger_attrs", attributes: { wattage: 45, made_by: ids.brand } });
    expect(one.taxonomies.find((x: { path: string }) => x.path === "launch").attributes).toBeNull();

    const populated = (await get(`/delivery/tx_item/${ids.charger}?taxonomies=1&populate=1`)).json();
    const attrs = populated.taxonomies.find((x: { path: string }) => x.path === "accessories.fast_chargers").attributes;
...
describe("delivery taxonomy — GraphQL", () => {
  ...
  it("filters lists, exposes entry taxonomies with attributes, and serves the trees", async () => {
    const q = await gql(`{
      txItems(taxonomy: ["tx_shop:accessories"], orderBy: [{ field: title, dir: ASC }]) {
        total items { title taxonomies { taxonomy path name attributeSet attributes } }
      }
      exact: txItems(taxonomy: ["tx_shop:accessories"], taxonomyExact: true) { total }
      taxonomies { uid }
      taxonomy(uid: "tx_shop") { name nodes { path depth attributeSet } }
      missing: taxonomy(uid: "zz_missing") { name }
    }`);
    expect(q.errors?.map((e) => e.message)).toBeUndefined();
    const d = q.data as unknown as {
      txItems: { total: number; items: Array<{ title: string; taxonomies: Array<{ path: string; attributes: { wattage?: number; made_by?: { id: string } } | null }> }> };
      exact: { total: number };
    ...
    const attrs = d.txItems.items[0]!.taxonomies.find((x) => x.path === "accessories.fast_chargers")!.attributes!;
    expect(attrs.wattage).toBe(45);
