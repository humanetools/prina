...
describe("Taxonomy (T2.5)", () => {
    ...
    const camera = await taxonomyNodeCreate.run(
      {
        ...
        name: "카메라",
        slug: "camera",
        entryComponentUid: "camera-specs",
      },
      t.ctx,
    );
    cameraId = camera.id;
  ...
  it("multi-taxonomy attach + attribute set value validation and storage", async () => {
      ...
      entrySetTaxonomies.run(
        {
          ...
          id: entryId,
          attachments: [
            { nodeId: cameraId, entryComponentValues: { sensor: "PLASMA" } },
          ],
        },
        t.ctx,
      ),
      ...
      entrySetTaxonomies.run(
        {
          typeUid: "article",
          id: entryId,
          attachments: [{ nodeId: rootId, entryComponentValues: { any: 1 } }],
        },
        t.ctx,
      ),
    ).rejects.toBeInstanceOf(ValidationError);
    ...
    const result = await entrySetTaxonomies.run(
      {
        ...
        attachments: [
          {
            nodeId: cameraId,
            entryComponentValues: { resolution: "20MP", sensor: "CMOS" },
          },
          { nodeId: rootId },
        ],
      },
    ...
    expect(cameraAttach.entryComponentUid).toBe("camera-specs");
    expect(cameraAttach.entryComponentValues).toEqual({ resolution: "20MP", sensor: "CMOS" });
  });
  ...
  it("node update — renames in place and swaps the attribute set; slug and path stay", async () => {
    ...
    expect(renamed.entryComponentUid).toBe(before.entryComponentUid); // untouched when not sent

    const detached = await taxonomyNodeUpdate.run({ nodeId: lensId, entryComponentUid: null }, t.ctx);
    expect(detached.entryComponentUid).toBeNull();
    expect(detached.name).toBe("Lenses & optics");
    await expect(
      taxonomyNodeUpdate.run({ nodeId: lensId, entryComponentUid: "no_such_component" }, t.ctx),
    ).rejects.toBeInstanceOf(ValidationError);
  });
