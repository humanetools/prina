...
import { entryRelations } from "../src/db/schema/index.js";
...
import { collectInverseEdges, populateValuesList } from "../src/modules/delivery/populate.js";
...
describe("delivery populate (?populate=1)", () => {
  ...
  it("render scope: ?populate=1 inlines relation targets with values (hand-off-01 §2) — off by default", async () => {
    await contentTypeCreate.run(
      { uid: "tag", name: "태그", definition: { fields: [{ name: "label", type: "text", required: true }], displayField: "label" } },
      t.ctx,
    );
    await contentTypeCreate.run(
      {
        ...
        definition: {
          fields: [
            { name: "title", type: "text", required: true },
            { name: "lead", type: "relation", target: "tag", relationKind: "manyToOne" },
    ...
    const t1 = await entryCreate.run({ typeUid: "tag", values: { label: "뉴스" } }, t.ctx);
    const t2 = await entryCreate.run({ typeUid: "tag", values: { label: "초안태그" } }, t.ctx);
    await publishEntry(t.ctx, "tag", t1.entry.id);
    const post = await entryCreate.run(
      { typeUid: "post", values: { title: "포스트1", lead: t1.entry.id, tags: [t1.entry.id, t2.entry.id] } },
      t.ctx,
    );
    await publishEntry(t.ctx, "post", post.entry.id);
    await templateSave.run(
      {
        typeUid: "post",
        liquid: "<h1>{{ values.title }}</h1><p class=\"lead\">{{ values.lead.values.label }}</p><ul>{% for g in values.tags %}<li>{{ g.values.label }}</li>{% endfor %}</ul><i>{{ values.lead }}</i>",
        css: "",
      },
      t.ctx,
    );

    // Without populate — relations stay ids (existing templates keep working)
    const raw = await app.inject({
      method: "GET",
      url: `/delivery/post/${post.entry.id}?format=html&embed=1&ws=${t.workspaceSlug}`,
    });
    expect(raw.statusCode).toBe(200);
    expect(raw.json().html).toContain(`<i>${t1.entry.id}</i>`);
    expect(raw.json().html).toContain('<p class="lead"></p>');

    // With populate — target values are in scope (depth 1); draft target t2 is dropped from the list
    const pop = await app.inject({
      method: "GET",
      url: `/delivery/post/${post.entry.id}?format=html&embed=1&populate=1&ws=${t.workspaceSlug}`,
    });
    expect(pop.statusCode).toBe(200);
    const html = pop.json().html as string;
    expect(html).toContain('<p class="lead">뉴스</p>');
    expect(html).toContain("<li>뉴스</li>");
    expect(html).not.toContain("초안태그");

    // Fragment mode honours it too
    const frag = await app.inject({
      method: "GET",
      url: `/delivery/post/${post.entry.id}?format=html&populate=1&ws=${t.workspaceSlug}`,
    });
    expect(frag.body).toContain("<li>뉴스</li>");

    // Editor preview mirrors the flag — and, being draft-inclusive, shows the draft target as well
    const preview = await templateRenderPreview.run(
      { typeUid: "post", entryId: post.entry.id, populate: true },
      t.ctx,
    );
    expect(preview.html).toContain('<p class="lead">뉴스</p>');
    expect(preview.html).toContain("<li>초안태그</li>");
    const previewRaw = await templateRenderPreview.run({ typeUid: "post", entryId: post.entry.id }, t.ctx);
    expect(previewRaw.html).toContain(`<i>${t1.entry.id}</i>`);
  });

  it("relations inside components / dynamic zones are indexed in entry_relations and feed inverse edges (hand-off-02)", async () => {
    await componentCreate.run(
      { uid: "blocks.grid", name: "Grid", definition: { fields: [{ name: "heading", type: "text" }, { name: "items", type: "relation", target: "writer", relationKind: "manyToMany", predicate: "mentions", inversePredicate: "mentionedIn", writeInverse: true }] } },
      t.ctx,
    );
    await componentCreate.run(
      { uid: "blocks.hero", name: "Hero", definition: { fields: [{ name: "author", type: "relation", target: "writer", relationKind: "manyToOne" }] } },
      t.ctx,
    );
    await contentTypeCreate.run(
      {
        ...
        definition: {
          fields: [
            { name: "title", type: "text", required: true },
            { name: "hero", type: "component", component: "blocks.hero" },
            { name: "sections", type: "dynamic_zone", components: ["blocks.grid"] },
          ],
          displayField: "title",
        },
      },
    ...
    const w1 = await entryCreate.run({ typeUid: "writer", values: { name: "블록작가" } }, t.ctx);
    await publishEntry(t.ctx, "writer", w1.entry.id);
    const w2 = await entryCreate.run({ typeUid: "writer", values: { name: "둘째작가" } }, t.ctx);
    const page = await entryCreate.run(
      {
        typeUid: "landing",
        values: {
          title: "랜딩",
          hero: { author: w1.entry.id },
          sections: [
            { __component: "blocks.grid", heading: "A", items: [w1.entry.id] },
            { __component: "blocks.grid", heading: "B", items: [w2.entry.id, w1.entry.id] },
          ],
        },
      },
      t.ctx,
    ...
    // entry_relations rows carry the relation path; repeated blocks concatenate in order
    const rows = await t.db
      .select({ field: entryRelations.field, to: entryRelations.toEntryId, position: entryRelations.position, predicate: entryRelations.predicate })
      .from(entryRelations)
      .where(eq(entryRelations.fromEntryId, page.entry.id));
    const byField = (f: string) => rows.filter((r) => r.field === f).sort((a, b) => a.position - b.position);
    expect(byField("hero.author").map((r) => r.to)).toEqual([w1.entry.id]);
    expect(byField("sections[blocks.grid].items").map((r) => r.to)).toEqual([w1.entry.id, w2.entry.id, w1.entry.id]);
    expect(byField("sections[blocks.grid].items")[0]!.predicate).toBe("mentions");
    expect(rows).toHaveLength(4);

    // Reverse lookup: writer w1's JSON-LD gets the inverse predicate from the page (published pages only)
    const dctx: DeliveryCtx = { db: t.db, services: t.ctx.services, workspace: { id: t.ctx.workspaceId, slug: "default", settings: {} }, includeDraft: false };
    expect(await collectInverseEdges(dctx, w1.entry.id)).toEqual({});
    await publishEntry(t.ctx, "landing", page.entry.id);
    const inverse = await collectInverseEdges(dctx, w1.entry.id);
    expect(inverse.mentionedIn?.map((s) => s.id)).toEqual([page.entry.id, page.entry.id]);
    // hero.author has no inverse predicate → not emitted
    expect(Object.keys(inverse)).toEqual(["mentionedIn"]);

    // Re-save with the grid removed → rows for that path disappear (values are the source of truth)
    await entryUpdate.run({ typeUid: "landing", id: page.entry.id, values: { title: "랜딩", hero: { author: w1.entry.id }, sections: [] } }, t.ctx);
    const after = await t.db.select({ field: entryRelations.field }).from(entryRelations).where(eq(entryRelations.fromEntryId, page.entry.id));
    expect(after.map((r) => r.field)).toEqual(["hero.author"]);
  });
});
