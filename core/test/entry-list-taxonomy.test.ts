...
beforeAll(async () => {
  ...
  const attach = (typeUid: string, id: string, attachments: Array<{ nodeId: string; entryComponentValues?: Record<string, unknown> }>) =>
    entrySetTaxonomies.run({ typeUid, id, attachments }, t.ctx);
  await attach("lt_page", ids.camera, [{ nodeId: nodes.vision, entryComponentValues: { megapixels: 12 } }, { nodeId: nodes.launch }]);
...
describe("entry.list — include=taxonomies", () => {
  ...
  it("gives every row — drafts too — the same shape the single-entry read returns; [] when nothing is attached", async () => {
    ...
    expect(byId.get(ids.camera)).toEqual([
      { nodeId: nodes.launch, name: "Launch", path: "launch", taxonomy: "lt-campaign", entryComponentValues: null, entryComponentUid: null, attributes: {} },
      { nodeId: nodes.vision, name: "Machine Vision", path: "business_unit.machine_vision", taxonomy: "lt-unit", entryComponentValues: { megapixels: 12 }, entryComponentUid: "lt_vision_attrs", attributes: {} },
    ]);
