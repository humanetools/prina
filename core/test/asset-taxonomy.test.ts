...
beforeAll(async () => {
  ...
  const node = async (ctx: TestContext, taxonomyUid: string, name: string, slug: string, parentId: string | null = null) =>
    (await taxonomyNodeCreate.run({ taxonomyUid, name, slug, parentId, attributeComponentUid: null }, ctx.ctx)).id;
...
describe("asset taxonomy — cascade", () => {
  it("deleting a node (with its subtree) removes the asset attachments under it", async () => {
    const parent = (await taxonomyNodeCreate.run({ taxonomyUid: "at_shop", name: "Temp", slug: "temp", parentId: null, attributeComponentUid: null }, t.ctx)).id;
    const child = (await taxonomyNodeCreate.run({ taxonomyUid: "at_shop", name: "Temp child", slug: "temp-child", parentId: parent, attributeComponentUid: null }, t.ctx)).id;
