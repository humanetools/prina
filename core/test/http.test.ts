...
describe("REST API (Phase 1 adapter)", () => {
  ...
  it("OpenAPI co-derives the type schema (T1.3 ②)", async () => {
    ...
    // GraphQL read plane (18-IMPL A) is part of the public delivery surface
    expect(doc.paths["/delivery/graphql"].post.requestBody).toBeDefined();
    expect(doc.paths["/delivery/graphql"].get).toBeDefined();
    expect(doc.paths["/delivery/graphql/schema"].get).toBeDefined();
  });
});
