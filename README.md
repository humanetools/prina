...
- **Delivery** — JSON API, **GraphQL** (read-only, schema generated from your types: filters, ordering, nested relations/media), Liquid HTML fragments (`?format=html`, `&populate=1` inlines related entries into the template scope), `embed.js` widgets, GA4 dataLayer
...
## Split deployment (admin inside, delivery outside)

Run the same image twice against the same Postgres and S3: one instance with `PRINA_PLANE=admin`
on your intranet (admin UI, management API, MCP management, embedding worker) and one or more with
`PRINA_PLANE=delivery` on the public network. A delivery instance answers only `/delivery/*`,
`/img/*`, `/mcp/delivery`, `/.well-known/*`, `/health` and `/` — every admin and management route
is 404 at the core, the admin UI is not served, and no background worker runs. Default `all`
keeps the single-instance behaviour. Deploy the admin instance first so migrations land before
delivery replicas roll.
