 ...
 * provider catalog with a connect modal, and the routing log. Model lists come from the providers
 * themselves (31-IMPL — ModelPicker / useProviderModels), not from a catalog in this file.
...
const CUSTOM_META: CatalogMeta = {
  ...
  sub: "Any endpoint that speaks the OpenAI API.", keyHint: "optional",
};
const CATALOG: Record<Target, Record<string, CatalogMeta>> = {
  lm: {
    anthropic: { name: "Anthropic", mono: "A", tint: "var(--accent-on)", tintBg: "var(--accent)", sub: "Claude Sonnet 5, Opus 5", docs: "https://console.anthropic.com/settings/keys", keyHint: "sk-ant-…" },
    openai: { name: "OpenAI", mono: "O", tint: "var(--accent-on)", tintBg: "var(--accent)", sub: "GPT-5.1, GPT-5 mini", docs: "https://platform.openai.com/api-keys", keyHint: "sk-proj-…" },
    gemini: { name: "Google Gemini", mono: "G", tint: "var(--accent-on)", tintBg: "var(--accent)", sub: "Gemini 2.5 Pro, Flash", docs: "https://aistudio.google.com/apikey", keyHint: "AIza…" },
    mistral: { name: "Mistral", mono: "M", tint: "var(--accent-on)", tintBg: "var(--accent)", sub: "Large, Small", docs: "https://console.mistral.ai/api-keys", keyHint: "…" },
    xai: { name: "xAI", mono: "X", tint: "var(--accent-on)", tintBg: "var(--accent)", sub: "Grok 4, Grok 4 mini", docs: "https://console.x.ai", keyHint: "xai-…" },
    llama: { name: "Meta Llama", mono: "L", tint: "var(--accent-on)", tintBg: "var(--accent)", sub: "Llama 4 Maverick, Scout", docs: "https://llama.developer.meta.com", keyHint: "LLM|…" },
    custom: { ...CUSTOM_META, modelHint: "llama-3.3-70b-instruct" },
  },
  ss: {
    voyage: { name: "Voyage AI", mono: "V", tint: "var(--accent-on)", tintBg: "var(--accent)", sub: "voyage-3.5, voyage-3-large", docs: "https://dashboard.voyageai.com/organization/api-keys", keyHint: "pa-…" },
    openai: { name: "OpenAI", mono: "O", tint: "var(--accent-on)", tintBg: "var(--accent)", sub: "text-embedding-3 family", docs: "https://platform.openai.com/api-keys", keyHint: "sk-proj-…" },
...
export function AiPage() {
  ...
  // connect modal: the provider's live model list, loaded once the key (or base URL) is in
  const addModels = useProviderModels();
  const addListArgs = () => adding ? {
    target: tab, provider: adding,
    ...(fKey ? { apiKey: fKey } : {}), ...(fUrl ? { baseUrl: fUrl } : {}), ...(fWsId ? { anthropicWorkspaceId: fWsId } : {}),
  } : null;
  const addKeyReady = adding === "custom" ? !!fUrl : adding === "custom" || fKey.length >= 8;
  useEffect(() => {
    if (!adding || !addKeyReady) return;
    const args = addListArgs();
    if (!args) return;
    // a short debounce — the key is typed or pasted, no button to press
    const h = setTimeout(() => {
      void addModels.load(args).then((out) => { if (out && out.models[0] && !fModel) setFModel(out.models[0].id); });
    }, 500);
    return () => clearTimeout(h);
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [adding, fKey, fUrl, fWsId, tab]);
  ...
  const openAdd = (provider: string) => {
    ...
    setAdding(provider); setFKey(""); setFModel(""); setFName(""); setFUrl(""); setFWsId("");
    addModels.reset();
    void m;
            ...
            return (
              <div key={e.id}>
                ...
                <div style={{
                  ...
                  <EntryModelPicker
                    target={tab} entry={e}
                    onChange={(model) => void saveChain(view.chain.map((x) => (x.id === e.id ? { ...x, model } : x)))}
                  />
            ...
            <div style={{ padding: "2.4rem 2.4rem 2.2rem" }}>
              ...
              <div className="form-fields" style={{ marginTop: "2rem" }}>
                ...
                <label className="field"><span>Model</span>
                  <ModelPicker
                    value={fModel} onChange={setFModel}
                    list={addModels.list} loading={addModels.loading} error={addModels.error}
                    onReload={addKeyReady ? () => { const a = addListArgs(); if (a) void addModels.load(a); } : undefined}
                    placeholder={addMeta.modelHint ?? "model-id"}
                  />
                </label>
                {adding === "anthropic" && (
...
/** Routing-order row: the connected entry's model, with its provider's live list (stored key, via entryId) */
function EntryModelPicker({ target, entry, onChange }: { target: Target; entry: ChainEntryView; onChange(model: string): void }) {
  const models = useProviderModels();
  const canList = !!entry.apiKeyMasked || entry.provider === "custom";
  const load = () => void models.load({ target, entryId: entry.id });
  // the list is fetched when the row first mounts; a failure just leaves the current model editable
  useEffect(() => { if (canList) load(); }, [entry.id]); // eslint-disable-line react-hooks/exhaustive-deps
  return (
    <div style={{ flex: "0 1 24rem", minWidth: "14rem" }}>
      <ModelPicker compact value={entry.model} onChange={onChange} list={models.list} loading={models.loading} error={models.error} onReload={canList ? load : undefined} />
    </div>
  );
}
