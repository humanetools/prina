/** Taxonomy (P9, UI for T2.5) — taxonomy list + folder tree of nodes (TaxonomyTree) + node editing (NodeDetail) */
...
export function TaxonomyPage() {
  ...
  return (
    <SectionLayout panelTitle="Taxonomy" panel={panel}>
      {uid ? <TaxonomyView key={uid} uid={uid} name={taxonomies?.find((t) => t.uid === uid)?.name ?? uid} /> : (
...
function TaxonomyView({ uid, name }: { uid: string; name: string }) {
  ...
  return (
    <>
      ...
      <div className="tax-layout">
        ...
        />
        <NodeDetail node={selected} taxonomyUid={uid} components={components ?? []} onSave={save} saving={updateNode.isPending} />
      </div>
    </>
  );
}
