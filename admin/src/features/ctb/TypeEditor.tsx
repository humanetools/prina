...
/**
 * Type name, editable in place (user, 2026-09-08): hover shows a pencil, click swaps the
 * heading for an input; Enter/blur saves through the same PUT the definition uses, Esc
 * cancels. The uid never changes — only the display name.
 */
function InlineTypeName({ name, onSave }: { name: string; onSave(next: string): Promise<void> }) {
  ...
  const commit = async () => {
    ...
    if (!next || next === name) { setEditing(false); setValue(name); return; }
    setBusy(true);
    try { await onSave(next); setEditing(false); } finally { setBusy(false); }
  };
  if (editing) {
    return (
      <input
        ref={inputRef}
        className="type-h1 type-h1-input"
        value={value}
  ...
  return (
    <span className="type-name">
      ...
      <button type="button" className="type-name-edit" title="Rename" aria-label={`Rename ${name}`} onClick={() => setEditing(true)}>
...
const LAST_TAB_KEY = "prina.ctb.tab";
const fieldKey = (f: FieldDef) => f.name;
const isTab = (t: string | null): t is Tab => t === "fields" || t === "semantic" || t === "seo" || t === "templates";
function readLastTab(): Tab {
  try { const t = sessionStorage.getItem(LAST_TAB_KEY); return isTab(t) ? t : "fields"; } catch { return "fields"; }
}

export function TypeEditor({ contentType }: { contentType: ContentType }) {
  const navigate = useNavigate();
  const [searchParams] = useSearchParams();
  // The editor remounts per type (CtbPage keys it by uid) — keep the chosen tab across that remount so
  // moving from type A's Templates to type B lands on B's Templates. ?tab= still wins when present.
  ...
  // manual order is the definition's order — dragging is off while a column sort is only a view
  const reorder = useRowReorder({ items: viewFields, keyOf: fieldKey, onReorder: saveDefinition, disabled: fieldSort !== null });
  ...
  return (
    <>
        ...
        <div>
          <div className="row-gap">
            <InlineTypeName
              ...
              onSave={async (name) => {
                setError(null);
                try { await update.mutateAsync({ name }); }
                catch (e) { setError([e instanceof ApiError ? e.message : "Rename failed"]); }
              }}
            />
      ...
      {tab === "fields" && (
        <>
          <div className="field-table" style={{ "--ft-cols": ftCols } as React.CSSProperties}>
            ...
            {reorder.items.map((f) => (
              <div key={f.name} {...reorder.rowProps(f.name)}>
              <div className="field-table-row" onClick={() => setModal({ kind: "edit", field: f })}>
                <span className={fieldSort ? "field-drag off" : "field-drag"} {...reorder.handleProps(f.name)}
                  title={fieldSort ? "Clear the column sort to reorder" : "Drag to reorder"}><GripIcon /></span>
      ...
      {tab === "templates" && (
        <>
          {/* key: the editor holds per-type state (sources, `loaded`, preview entry) — switching types in the
              sidebar must remount it, or type A's liquid/css and entry stay on screen under type B (hand-off-01 §1) */}
          <TemplateEditor key={contentType.uid} typeUid={contentType.uid} mode="embedded" />
