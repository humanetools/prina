...
export function useKgViewport({
  ...
  /** Set on pointerup when the gesture moved — the following click must not select/open anything */
  ...
  useEffect(() => {
    ...
    const up = () => {
      ...
      // The click event arrives after pointerup — remember whether this gesture was a drag so
      // clickGuard can swallow it (drag.current is already gone by then)
  ...
  const clickGuard = (fn: () => void) => () => {
    if (drag.current?.moved || suppressClick.current) {
      suppressClick.current = false;
      return;
    }
    fn();
