...
import { useContentTypes, useLocales, useTaxonomies } from "../../hooks/queries";
...
export function ApiExplorerPanel({ open, onClose }: { open: boolean; onClose(): void }) {
  ...
  // the Taxonomy screen has no type in view — open on the workspace surfaces where its endpoints live
  const [tab, setTab] = useState<"type" | "workspace" | "graphql">(location.pathname.startsWith("/taxonomy") ? "workspace" : "type");
  // GraphQL (18-IMPL A) — one query box against /delivery/graphql; the default follows the type
  const [gqlQuery, setGqlQuery] = useState<string | null>(null);
  ...
  const endpoints = useMemo<EndpointDef[]>(() => {
    ...
    // Taxonomy examples follow the first taxonomy that has a node (33-IMPL)
    const tax = taxonomies?.[0]?.uid ?? "{taxonomyUid}";
    return [
      ...
      {
        label: "Entries · by taxonomy",
        group: "type",
        scope: "list",
        hint: "taxonomy=<uid>:<node.path> keeps entries under that node (descendants included; taxonomyExact=1 for the node alone). taxonomies=1 adds each entry's nodes with their attributes and entry-component values.",
        uri: `/delivery/${effectiveType}?ws=${ws}&taxonomy=${tax}:{node.path}&taxonomies=1`,
      },
      // ── workspace-wide surfaces ──
      {
        label: "Taxonomies · list",
        group: "workspace",
        hint: "Every taxonomy with its attributeFields — the attributes each node carries values for.",
        uri: `/delivery/taxonomies?ws=${ws}`,
      },
      {
        label: "Taxonomy · tree",
        group: "workspace",
        hint: "Flat, path-ordered nodes (assemble from parentId). Each node: attributes (its own values) + entryComponent (what entries under it fill in).",
        uri: `/delivery/taxonomies/${tax}?ws=${ws}`,
      },
      {
        label: "Search",
        group: "workspace",
  ...
  }, [effectiveType, sampleId, routeEntryId, ws, locales, types, taxonomies]);
  ...
  /** camelCase list field the schema generator produces for a type uid (naive plural, same rules as core) */
  const gqlListField = (uid: string) => {
    const parts = uid.split(/[^0-9A-Za-z]+/).filter(Boolean).map((x) => x[0]!.toUpperCase() + x.slice(1)).join("");
    const c = parts[0]!.toLowerCase() + parts.slice(1);
    if (/[^aeiou]y$/i.test(c)) return c.slice(0, -1) + "ies";
    if (/(s|x|z|ch|sh)$/i.test(c)) return c + "es";
    return c + "s";
  };
  const gqlDefault = () => {
    const t = types?.find((x) => x.uid === effectiveType);
    if (!t) return "{ __schema { queryType { fields { name } } } }";
    const scalarish = t.definition.fields.filter((f) => ["text", "uid", "number", "boolean", "date", "enum"].includes(f.type)).slice(0, 4).map((f) => f.name);
    const media = t.definition.fields.find((f) => f.type === "media");
    const sel = ["id", ...scalarish, ...(media ? [`${media.name} { url alt }`] : [])].join(" ");
    return t.kind === "single"
      ? `{ ${gqlListField(t.uid).replace(/(ies|es|s)$/, "")} { ${sel} } }`
      : `{ ${gqlListField(t.uid)}(limit: 5, orderBy: [{ field: publishedAt, dir: DESC }]) { total items { ${sel} } } }`;
  };
  const runGraphql = async () => {
    const q = gqlQuery ?? gqlDefault();
    const uri = `/delivery/graphql?ws=${ws}`;
    setBusy(true);
    setResult(null);
    try {
      const res = await fetch(uri, { method: "POST", credentials: "same-origin", headers: { "content-type": "application/json" }, body: JSON.stringify({ query: q }) });
      const raw = await res.text();
      let body = raw;
      try { body = JSON.stringify(JSON.parse(raw), null, 2); } catch { /* keep raw */ }
      setResult({ uri, status: res.status, contentType: res.headers.get("content-type") ?? "", body: body.length > VIEWER_CAP ? body.slice(0, VIEWER_CAP) : body });
    } catch {
      setResult({ uri, status: 0, contentType: "", body: "Request failed" });
    } finally {
      setBusy(false);
  ...
  const copyRaw = (key: string, text: string) => {
    void copyText(text).then((ok) => {
      // ✓ only when the clipboard actually took it; otherwise say so instead of pretending
      setCopied(ok ? key : `!${key}`);
      setTimeout(() => setCopied(null), 1500);
    });
  };
  // List rows copy the full URL a frontend would call; the result pane copies the response body
  const copyUri = (key: string, uri: string) => copyRaw(key, `${window.location.origin}${uri}`);
    ...
    <aside className="assistant-panel api-panel">
      ...
      <div className="assistant-msgs">
          ...
          {result ? (
            <>
              <div className="api-endpoint-head">
                ...
                <button className="api-result-copy" title="Copy response body"
                  onClick={() => copyRaw(RESULT_COPY_KEY, result.body)}>
                  {copied === RESULT_COPY_KEY ? "✓" : copied === `!${RESULT_COPY_KEY}` ? "✗" : <IconCopy size="1.3rem" />}
                </button>
              </div>
              <pre>{result.body}</pre>
            </>
        ...
        <div className="tabs api-tabs">
          {([["type", "Content type"], ["workspace", "Workspace"], ["graphql", "GraphQL"]] as const).map(([key, label]) => (
        ...
        {tab === "graphql" && (
          <div className="api-endpoint">
            <div className="api-endpoint-head">
              <strong>GraphQL · read</strong>
              <span className="api-method">POST</span>
              <a className="muted" href={`/delivery/graphql/schema?ws=${ws}`} target="_blank" rel="noreferrer" style={{ marginLeft: "auto" }}>schema (SDL) ↗</a>
            </div>
            <div className="muted api-hint">
              One schema per workspace, generated from your types: <code>{gqlListField(effectiveType || "type")}(where, orderBy, limit, offset)</code>,{" "}
              <code>{"<type>(id)"}</code>, single types by name. Published entries only (add <code>&draft=</code> token for drafts). Read-only.
            </div>
            <label className="field" style={{ marginTop: "0.6rem" }}>
              <span>Query</span>
              <textarea rows={7} value={gqlQuery ?? gqlDefault()} spellCheck={false}
                style={{ fontFamily: "var(--font-mono)", fontSize: "1.2rem" }}
                onChange={(e) => setGqlQuery(e.target.value)} />
            </label>
            <div className="row-gap">
              <button className="btn btn-sm btn-primary" disabled={busy} onClick={() => void runGraphql()}>
                <IconPlayerPlay size="1.3rem" /> Run
              </button>
              <button className="btn btn-sm" onClick={() => setGqlQuery(null)}>Reset to example</button>
            </div>
          </div>
        )}
        {tab === "type" && (
            ...
            <div className="api-endpoint-row">
              <span className="api-uri-wrap">
                ...
                <button className="api-copy" title="Copy full URL" onClick={() => copy(e)}>
                  {copied === e.label ? "✓" : copied === `!${e.label}` ? "✗" : <IconCopy size="1.3rem" />}
                </button>
              </span>
