...
import { EntryClassificationContext } from "./EntryClassificationContext";
...
export function ContentEditPage({ mode }: { mode: "create" | "edit" }) {
  ...
  // Content · Taxonomy · SEO — the tab lives in the URL so a refresh or a shared link lands on it
  ...
  // the asset picker reads the entry's classification from context (25-IMPL)
  const classification = useMemo(
    () => (detail?.taxonomies ?? []).map((t) => ({ nodeId: t.nodeId, name: t.name, spec: `${t.taxonomy}:${t.path}` })),
    [detail?.taxonomies],
  );
  ...
  const seoTab = mode === "edit" && !!detail && !!contentType.options?.seo?.enabled && !isChild;
  const tabs = mode === "edit" && detail
    ? [
        { key: "content", label: "Content" },
        { key: "taxonomy", label: "Taxonomy", count: detail.taxonomies.length },
        ...(seoTab ? [{ key: "seo", label: "SEO" }] : []),
      ]
    : [];
  const asked = searchParams.get("tab");
  const tab = tabs.some((t) => t.key === asked) ? asked! : "content";

  return (
    <EntryClassificationContext.Provider value={classification}>
    <SectionLayout
    ...
    >
      <div className="edit-layout">
        <div className="edit-main">
          ...
          {tabs.length > 0 && (
            <div className="tabs entry-tabs">
              {tabs.map((t) => (
                <button
                  key={t.key} className={tab === t.key ? "tab active" : "tab"}
                  onClick={() => setSearchParams(t.key === "content" ? {} : { tab: t.key }, { replace: true })}
                >
                  {t.label}
                  {"count" in t && t.count ? <span className="tab-count">{t.count}</span> : null}
                </button>
              ))}
            </div>
          )}
          {tab === "content" && (
            <>
              ...
              {mode === "edit" && detail && <VariantsMatrix contentType={contentType} detail={detail as EntryDetail} />}
            </>
          )}
          {tab === "taxonomy" && detail && <TaxonomySection typeUid={typeUid!} detail={detail} />}
          {tab === "seo" && detail && <SeoSection typeUid={typeUid!} detail={detail} contentType={contentType} />}
        </div>
    ...
    </EntryClassificationContext.Provider>
  );
}
