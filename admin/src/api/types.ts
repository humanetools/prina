...
export interface EntryDetail {
  ...
  taxonomies: Array<{
    ...
    /** the node's own attribute values (33-IMPL) */
    attributes: Record<string, unknown>;
    /** taxonomy uid — `<taxonomy>:<path>` is the asset / delivery filter spec */
    taxonomy: string;
  }>;
}
...
/** Mirror of @prina/shared TaxonomyAttributeType (33-IMPL) */
export enum TaxonomyAttributeType {
  Text = "text",
  Number = "number",
  Boolean = "boolean",
}
/** Attribute definition on a taxonomy — every node carries a value per field */
...
export interface TaxonomyNode {
  ...
  slug: string;
  path: string;
  /** component uid entries under this node fill in (was "attribute set" before 33-IMPL) */
  entryComponentUid: string | null;
  /** the node's own values for the taxonomy's attributeFields */
  attributes: Record<string, unknown>;
  position: number;
}
...
export interface AssetDetail extends Asset {
  ...
  taxonomies: AssetTaxonomyRef[];
  deletable: boolean;
}

/** DAM folder tree (24-IMPL) — flat, parents first; `count` = assets directly in the folder */
...
export interface AssetFolderTree {
  items: AssetFolderNode[];
  /** assets filed directly under `/` */
  rootCount: number;
  total: number;
}

/** A taxonomy node an asset sits in (23-IMPL) — `taxonomy` is the taxonomy uid */
export interface AssetTaxonomyRef {
  taxonomy: string;
  nodeId: string;
  name: string;
  slug: string;
...
/** REST access-token scopes (mirror of @prina/shared ApiScope / ApiScopeLevel — keep in sync) */
...
/** Custom-mode grants (mirror of @prina/shared ApiGrant* — keep in sync) */
...
export interface ApiGrant {
  item: ApiGrantItem;
  level: ApiGrantLevel;
  /** entries / publish / import only — content type uids; omitted = all */
  types?: string[];
}
...
export interface McpToken {
  ...
  /** management: REST surface per resource group (server resolves legacy NULL to { content: edit }); delivery: null */
  scopes: ApiScopes | null;
