...
/** Node attribute diagram — label + value rows + sliders badge (Taxonomy › Attribute, 33-IMPL) */
export function AttributeArt() {
  const row = (y: number, hue: number, barW: number, valW: number) => (
    <>
      <rect x="98" y={y} width="14" height="14" rx="4" className="hero-tile" style={tileStyle(hue)} />
      <rect x="118" y={y + 4.5} width={barW} height="5" rx="2.5" fill="var(--surface-3)" />
      <rect x={176 - valW} y={y + 1} width={valW} height="12" rx="3" fill="var(--surface)" stroke="var(--surface-3)" />
    </>
  );
  return (
    <StackedCards>
      <rect x="98" y="44" width="36" height="6" rx="3" fill="var(--border-strong)" />
      {row(60, 264, 26, 22)}
      {row(82, 152, 30, 16)}
      {row(104, 40, 22, 12)}
      <Badge icon={
        <g stroke="var(--accent-on)" strokeWidth="1.6" strokeLinecap="round">
          <path d="M167 122h12M167 126h12M167 130h12" />
          <circle cx="171" cy="122" r="1.6" fill="var(--accent-on)" stroke="none" />
          <circle cx="176" cy="126" r="1.6" fill="var(--accent-on)" stroke="none" />
          <circle cx="170" cy="130" r="1.6" fill="var(--accent-on)" stroke="none" />
        </g>
      } />
    </StackedCards>
  );
